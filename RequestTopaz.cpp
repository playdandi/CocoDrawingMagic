    ...
    for (int i = 0 ; i < spriteClassScroll->spriteObj.size() ; i++)
    {
        if (spriteClassScroll->spriteObj[i]->name.substr(0, 26) == "button/btn_yellow_mini.png")
        {
                ...
                (int)p.x >= 0 && (int)p.y >= 0 && (int)p.x <= size.width && (int)p.y <= size.height)
            {
                ...
                Network::HttpPost(param, URL_REQUEST_TOPAZ, this, httpresponse_selector(RequestTopaz::onHttpRequestCompleted), temp);
...
void RequestTopaz::XmlParseResult(xml_document *xmlDoc, int friendKakaoId)
{
